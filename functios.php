...
function finanmotors_handle_pdf_send( WP_REST_Request $request ) {
	try {
		...
		if ( ! empty( $client_phone ) && defined( 'TWILIO_ACCOUNT_SID' ) && defined( 'TWILIO_AUTH_TOKEN' ) && defined( 'TWILIO_FROM' ) ) {
			$tw_sid = TWILIO_ACCOUNT_SID;
			$tw_token = TWILIO_AUTH_TOKEN;
			$tw_from = TWILIO_FROM;
			$use_whatsapp = defined( 'TWILIO_USE_WHATSAPP' ) && TWILIO_USE_WHATSAPP;

			// Build public URL for uploaded PDF (uploads baseurl + relative path)
			$upload = wp_upload_dir();
			$public_url = '';
			if ( ! empty( $file_path ) && file_exists( $file_path ) ) {
				$uploads_base = trailingslashit( $upload['basedir'] );
				$uploads_url_base = trailingslashit( $upload['baseurl'] );
				// If our file is inside the uploads dir, compute its public URL
			...
			$sms_body = 'Tu cotización Finan Motors';
			if ( ! empty( $marca ) || ! empty( $modelo ) ) {
				$sms_body .= ': ' . trim( $marca . ' ' . $modelo );
			}
			if ( ! empty( $public_url ) ) {
				// Prefer MMS/Media if supported: include MediaUrl; otherwise body includes the link
				$sms_body .= ' - Ver PDF: ' . $public_url;
			}

			$to = $client_phone;
			$from = $use_whatsapp ? ('whatsapp:' . $tw_from) : $tw_from;
			$to_field = $use_whatsapp ? ('whatsapp:' . $to) : $to;
			...
			if ( ! empty( $public_url ) && ! $use_whatsapp ) {
				// For SMS/MMS use MediaUrl (WhatsApp via Twilio also supports media but requires different handling)
				$post_fields['MediaUrl'] = $public_url;
			}
			...
			$log( 'Twilio send attempted. HTTP code: ' . $http_code . ', curl_err: ' . $curl_err . ', response: ' . substr( $result, 0, 500 ) );
		} elseif ( ! empty( $client_phone ) ) {
			$log( 'Client phone provided but Twilio credentials not configured; skipping SMS send.' );
		}
		...
		if ( ! $sent_company && ! $sent_client ) {
			...
			$log( 'Neither email sent successfully.' );
